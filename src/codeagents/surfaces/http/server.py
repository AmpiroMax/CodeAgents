...
from codeagents.observability.request_log import ServiceRequestLogger, ServiceRequestLogEntry
from codeagents.observability.resource_metrics import collect_resource_snapshot
from codeagents.core.runtime.openai_client import RuntimeErrorWithHint
from codeagents.core.stream_events import StreamErrorEvent, stream_event_to_json
...
class AgentRequestHandler(BaseHTTPRequestHandler):
    ...
    def _get_inference_models(self) -> None:
        ...
        ollama_models = []
        try:
            from codeagents.core.budget.params import ensure_for_models
    ...
    def _get_metrics_history(self) -> None:
        from codeagents.observability.metrics_sampler import get_global_sampler
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if self.path == "/chat/confirm":
                from codeagents.core.orchestrator import submit_confirmation
    ...
    def _stream_metrics_ndjson(self, started: float) -> None:
        ...
        from codeagents.observability.metrics_sampler import get_global_sampler, stream_snapshots
    ...
    def _index_new_messages_async(self, chat: Chat, new_messages: list[Any]) -> None:
        ...
        def _run() -> None:
            try:
                from codeagents.rag.chat_embeddings import index_pending_chat_messages
                from codeagents.stores.chat import default_chats_dir
                from codeagents.core.config import load_app_config
...
def serve(
    *,
    ...
    try:
        from codeagents.core.budget.params import ensure_for_models, PARAMS_DIR
